...
switch ($mode) {
    ...
    case "ratinggreaterorequal":
        ...
        break;
    case "directorname":
        $sql = 'SELECT name FROM movies JOIN movies_directors AS md ON movies.id = md.movie_id JOIN directors ON directors.id = md.director_id WHERE directors.last_name OR directors.first_name LIKE "%' . $strSearch . '%" LIMIT ' . $pageLimit . ' OFFSET ' . $offset;
        break;
    default:
